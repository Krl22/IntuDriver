...
function History() {
  ...
  const [originLabels, setOriginLabels] = React.useState<Record<string, string>>({}); // origen
  ...
  React.useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (err) => {
        ...
        try {
          console.debug("Fallo consulta con índice en History; usando fallback", err);
          const q2 = query(collection(firestore, "rides"), where("driverId", "==", user.uid));
  ...
  const reverseGeocode = React.useCallback(
    async (lat: number, lng: number): Promise<string | null> => {
      try {
        ...
        const city = data2.address?.city || data2.address?.town || data2.address?.village || data2.address?.municipality;
        const road = data2.address?.road || data2.address?.suburb;
        if (city && road) return `${road}, ${city}`;
        if (city) return city;
        if (typeof data2.display_name === "string") return formatAddress(data2.display_name);
        return null;
      } catch {
        return null;
      }
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      ...
      for (const r of rides) {
        ...
        } else if (typeof dest?.lat === "number" && typeof dest?.lng === "number" && !nextDest[id]) {
          tasks.push(
        ...
        if (typeof origin?.lat === "number" && typeof origin?.lng === "number" && !nextOrigin[id]) {
          tasks.push(
  ...
  return (
    <div className="space-y-4">
      ...
      {rides.length === 0 ? (
        <p className="text-green-700 text-sm">Aún no tienes viajes completados.</p>
      ) : loading ? (
        <div className="space-y-3">
          {rides.slice(0, Math.min(rides.length, 5)).map((_, i) => (
            <div key={i} className="p-3 rounded-lg bg-white border border-green-100 animate-pulse">
        ...
        <div className="space-y-3">
          {rides.map((r) => {
            const originLabel = originLabels[r.id] ?? (typeof r.data.origin?.lat === "number" && typeof r.data.origin?.lng === "number" ? `Lat ${r.data.origin.lat.toFixed(4)}, Lng ${r.data.origin.lng.toFixed(4)}` : "(sin datos)");
            ...
            return (
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="text-green-800 font-medium truncate">{originLabel}</p>
                  <p className="text-green-600 text-center leading-none">|</p>
                  <p className="text-green-800 font-medium truncate">{destLabel}</p>
